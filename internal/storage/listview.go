...
func (i accountItem) Description() string {
	return fmt.Sprintf("%s · %s · %s · %s", i.account.Kind, i.account.AccessTier, i.account.Region, i.account.ResourceGroup)
}
...
func newListView(p provider.StorageProvider) listView {
	l := list.New(nil, list.NewDefaultDelegate(), 0, 0)
	l.Title = "Storage Accounts"
...
func (v listView) fetchList() tea.Cmd {
	...
	return func() tea.Msg {
		...
		defer cancel()
		accounts, err := p.ListStorageAccounts(ctx)
	...
	case tea.KeyMsg:
		...
		case "enter":
			if !v.loading && v.list.FilterState() != list.Filtering {
				if sel, ok := v.list.SelectedItem().(accountItem); ok {
					return v, func() tea.Msg {
						return router.PushMsg{Screen: newDetailView(sel.account.ID, v.provider)}
					}
				}
			}
		}
...
func (v listView) View() string {
	if v.loading {
		return v.spinner.View() + " Loading storage accounts..."
	}
	if v.err != nil {
